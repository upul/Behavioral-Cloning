...
In this project, we were working on a regression problem in the context of self-driving cars. In the initial phase of the project, we mainly focused on finding a suitable network architecture and trained a model using our own dataset. According to *MSE* of my model it worked well. However, it didn't perform well when we deploy the model in our simulator. So was a clear indication of the *MSE* is not a good metrics to assess the performance this project. 
